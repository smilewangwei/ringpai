...
Sub Optimization()
    Dim arr
    arr = Array("序号", "产品分类", "产品名称", "产品规格", "期初单价", "期初数量", "期初金额", "入库单价", "入库数量", "采购金额", "出库单价", "出库数量", "销售数量", "销售成本", "盘点损益调整", "损益金额", "期末单价", "期末数量", "期末金额", "盘点实存", "校验")
    ...
    '"&DicKyes(0)&"
    Dim Title_MaxRow
    Dim Title_MinRow As Integer
    Title_MinRow = ChearCol(2, "产品名称") + 1
    Title_MaxRow = ActiveSheet.Range(ChearCol(1, "产品名称") & "65536").End(xlUp).Row + 30

    Range(DicKyes(0) & Title_MinRow).Formula = "=row()"
    Range(DicKyes(6) & Title_MinRow).Formula = "=" & DicKyes(4) & "2*" & DicKyes(5) & Title_MinRow
    Range(DicKyes(7) & Title_MinRow).Formula = "=SUMIF('订单入库管理-03'!F:F," & DicKyes(2) & "3,'订单入库管理-03'!O:O)"
    Range(DicKyes(8) & Title_MinRow).Formula = "=SUMIF('订单入库管理-03'!F:F," & DicKyes(2) & "3,'订单入库管理-03'!P:P)"
    Range(DicKyes(9) & Title_MinRow).Formula = "=SUMIF('订单入库管理-03'!F:F," & DicKyes(2) & "3,'订单入库管理-03'!Q:Q)"
    Range(DicKyes(10) & Title_MinRow).Formula = "=IF((" & DicKyes(5) & "2+" & DicKyes(8) & "2)=0,0,(" & DicKyes(6) & "2+" & DicKyes(9) & "2)/(" & DicKyes(5) & "2+" & DicKyes(8) & "2))"
    Range(DicKyes(11) & Title_MinRow).Formula = ""
    Range(DicKyes(12) & Title_MinRow).Formula = "=" & DicKyes(11) & Title_MinRow
    Range(DicKyes(13) & Title_MinRow).Formula = "=" & DicKyes(10) & "2*" & DicKyes(12) & Title_MinRow
    Range(DicKyes(14) & Title_MinRow).Formula = ""
    Range(DicKyes(15) & Title_MinRow).Formula = "=" & DicKyes(10) & "2*" & DicKyes(14) & Title_MinRow
    Range(DicKyes(16) & Title_MinRow).Formula = "=IFERROR(" & DicKyes(18) & "2/" & DicKyes(17) & "2,0)"
    Range(DicKyes(17) & Title_MinRow).Formula = "=" & DicKyes(5) & "2+" & DicKyes(8) & "2-" & DicKyes(11) & "2+" & DicKyes(14) & Title_MinRow
    Range(DicKyes(18) & Title_MinRow).Formula = "=" & DicKyes(10) & "2*" & DicKyes(17) & Title_MinRow
    Range(DicKyes(19) & Title_MinRow).Formula = ""
    Range(DicKyes(20) & Title_MinRow).Formula = "=" & DicKyes(19) & "2-" & DicKyes(17) & Title_MinRow
